...
def transform_imported_work_orders(raw_df: pd.DataFrame, default_location: str) -> tuple[pd.DataFrame, list[str]]:
    ...
    parsed_dates = parsed_dates - pd.Timedelta(days=1)
...
def _ta_retrieve(
    wo_filtered, rfm_filtered, question, history=False,
    wo_history=None, rfm_history=None,
):
    """Search filtered events; resolve status only from permitted full history."""
    closed = {"COMPLETED", "COMP", "CLOSED", "CLOSE", "RTS", "DONE",
              "CANCL", "CANCELLED", "CANCELED"}
    opened = {"OPEN", "APPR", "WIP", "WMATL", "HOLD", "INPRG", "WAPPR",
              "DRAFT", "SUBMITTED", "PO", "WSCH", "SCHED"}
    q = question.lower().strip()
    bay = re.search(r"\bbay\s*#?\s*(\d+)\b", q)
    cap = re.search(r"\b(?:cap|capsule)\s*#?\s*(\d+)\b", q)
    requested_bay = f"Bay {int(bay.group(1))}" if bay else ""
    requested_cap = f"Cap #{int(cap.group(1))}" if cap else ""
    ids = re.findall(r"\b(wo|rfm)\s*#?\s*(\d+)\b", q)
    kinds = {"WO", "RFM"}
    if re.search(r"\brfms?\b", q) and not re.search(r"\bwos?\b", q):
        kinds = {"RFM"}
    elif re.search(r"\bwos?\b|\bwork\s+orders?\b", q):
        kinds = {"WO"}
    wants_open = bool(re.search(r"\bopen\b", q))
    wants_closed = bool(re.search(r"\bclosed\b|\bcompleted\b", q))
    requested_state = "Open" if wants_open and not wants_closed else (
        "Closed" if wants_closed and not wants_open else "")
    # An ID question must show the actual status, even "is WO123 open?".
    state_filter = requested_state if not ids else ""
    q = re.sub(r"\bbay\s*#?\s*\d+\b|\b(?:cap|capsule)\s*#?\s*\d+\b", "", q)
    q = re.sub(r"\b(wo|rfm)\s*#?\s*\d+\b", "", q)
    stop = set(("a an the is are was were be been of to for in on at and or with "
                "about me show tell give list summarize summary please what which how "
                "many all any wo wos rfm rfms work order orders status latest current "
                "history resolution description happened update updates this that these "
                "those it its have has do does can you i my we our there currently "
                "open closed completed still").split())
    ...
    def key(value, kind):
        value = re.sub(rf"^{kind}\s*#?\s*", "", clean(value), flags=re.I)
        return re.sub(r"^(\d+)\.0+$", r"\1", value).upper()

    def prepare(frame, kind):
        if frame is None or frame.empty or kind not in frame:
            return pd.DataFrame()
        out = frame.copy().reset_index(drop=True)
        out["__id"] = out[kind].map(lambda v: key(v, kind))
        out = out[out["__id"].ne("")].copy()
        out["__ts"] = _parse_ts(out)
        out["__order"] = range(len(out))
        return out.sort_values(["__ts", "__order"], kind="stable", na_position="first")

    def slot(value, prefix):
        m = re.fullmatch(rf"(?:(?:{prefix})\s*#?\s*)?(\d+)(?:\.0+)?",
                         clean(value), re.I)
        return int(m.group(1)) if m else None

    candidates = []
    available = 0
    for kind, filtered, full in (
        ("WO", wo_filtered, wo_history), ("RFM", rfm_filtered, rfm_history),
    ):
        if kind not in kinds:
            continue
        events = prepare(filtered, kind)
        all_events = prepare(full, kind)
        if events.empty:
            continue
        available += events["__id"].nunique()
        latest = {} if all_events.empty else {
            r["__id"]: r for _, r in all_events.drop_duplicates("__id", keep="last").iterrows()
        }
        id_targets = {key(number, kind) for label, number in ids if label.upper() == kind}
        for record_id, group in events.groupby("__id", sort=False):
            if ids and record_id not in id_targets:
                continue
            relevant = group
            if bay:
                relevant = relevant[
                    relevant.get("Bay", pd.Series("", index=relevant.index))
                    .map(lambda v: slot(v, "bay")).eq(int(bay.group(1)))
                ]
            if cap:
                relevant = relevant[
                    relevant.get("Capsule", pd.Series("", index=relevant.index))
                    .map(lambda v: slot(v, "cap|capsule")).eq(int(cap.group(1)))
                ]
            if relevant.empty:
                continue
            current = latest.get(record_id)
            status = clean(current.get("Status")) if current is not None else ""
            upper = status.upper()
            state = ("Closed" if upper in closed else "Open" if upper in opened
                     else "Note" if upper == "NOTE" else "Unknown")
            # NOTE describes a note, not evidence that an equipment fault was repaired.
            searchable = relevant[[c for c in TA_FIELDS if c in relevant]].fillna("").astype(str)
            body = " ".join(searchable.to_numpy().ravel()).lower()
            score = sum(bool(re.search(r"\b" + re.escape(t) + r"\b", body)) for t in set(terms))
            if terms and not ids and score == 0:
                continue
            chosen = relevant if history else relevant.tail(1)
            for _, event in chosen.iterrows():
                row = {"Type": kind, kind: record_id,
                       "Status": status or "Unknown", "CurrentState": state}
                for col in ("Title", "Resolution", "Description", "Date", "AssignedTo"):
                    row[col] = clean((event if history or current is None else current).get(col))
                for col in ("Location", "Bay", "Capsule", "CapsuleID"):
                    row[col] = clean(event.get(col))
                row["MatchedDate"] = clean(event.get("Date"))
                row["LatestUpdate"] = clean(current.get("CreatedAt")) or clean(current.get("Date")) if current is not None else ""
                if history:
                    row["EventStatus"] = clean(event.get("Status"))
                row["__score"] = score
                row["__sort"] = current["__ts"] if current is not None else event["__ts"]
                candidates.append(row)

    pool = pd.DataFrame(candidates)
    counts = {"Open": 0, "Closed": 0, "Note": 0, "Unknown": 0}
    if not pool.empty:
        unique = pool.drop_duplicates(["Type", "WO", "RFM"] if {"WO", "RFM"}.issubset(pool)
                                      else ["Type", "WO" if "WO" in pool else "RFM"])
        counts.update({k: int(v) for k, v in unique["CurrentState"].value_counts().items()})
        if state_filter:
            pool = pool[pool["CurrentState"].eq(state_filter)]
        pool = pool.sort_values(["__score", "__sort"], ascending=False, kind="stable",
                                na_position="last")
    matched = len(pool)
    records = []
    for _, row in pool.head(TA_MAX_ROWS).iterrows():
        ...
        for column, value in row.items():
            ...
            value = clean(value)
            if value:
                record[column] = value[:TA_CELL_CHARS] + " [truncated]" if len(value) > TA_CELL_CHARS else value
        if len(json.dumps(records + [record], ensure_ascii=False).encode("utf-8")) > TA_CONTEXT_BYTES:
            break
        records.append(record)
    return records, {
        "available_records": int(available), "matched_rows": matched,
        "sent_rows": len(records), "requested_bay": requested_bay,
        "requested_cap": requested_cap, "requested_state": state_filter,
        "record_label": "work orders" if kinds == {"WO"} else "RFMs" if kinds == {"RFM"} else "WO/RFM records",
        "state_counts_before_cap": counts,
        "count_scope": "active filters and explicit Bay/Capsule constraints",
        "keyword_search": bool(terms and not ids),
        "truncated": len(records) < matched,
    }


def _ta_no_results(coverage):
    """Only assert zero open when local status evidence permits it."""
    where = " / ".join(v for v in (coverage.get("requested_bay"),
                                   coverage.get("requested_cap")) if v)
    where = f" for {where}" if where else ""
    state = coverage.get("requested_state", "")
    label = coverage.get("record_label", "WO/RFM records")
    counts = coverage.get("state_counts_before_cap", {})
    if state and not coverage.get("keyword_search"):
        if counts.get("Unknown", 0):
            return (f"No confirmed {state.lower()} {label} found{where} within the active filters. "
                    f"{counts['Unknown']} record(s) have unknown status, so this is not a confirmed zero.")
        return f"No {state.lower()} {label} found{where} within the active filters."
    return "No relevant rows found within the active filters. Try a WO/RFM number or adjust the filters."
...
def _ta_answer(question, records, coverage, model, api_key):
    ...
    instructions = (
        ...
        "Status and CurrentState come from the latest loaded record in permitted site history. "
        "Bay/Capsule and MatchedDate identify the matching event; LatestUpdate dates the status evidence. "
        "EventStatus in history mode is historical, not the current status. "
        "NOTE means a note, not proof of closure or repair. Never label unknown status open. "
        "State counts are calculated locally before the row cap within active filters; use them for totals. "
        "Do not present closed historical WOs as active work, or assume closure proves equipment repaired. "
...
def render_turnover_assistant(
    ...
    for frame in (wo_filtered, rfm_filtered, wo_history, rfm_history):
        safe = frame[[c for c in (*TA_FIELDS, "CreatedAt") if c in frame]].fillna("").astype(str)
        ...
        with st.chat_message("assistant"):
            ...
            if states == {"CLOSED"}:
                st.markdown(
                    ...
                    "font-size:.8rem;'>✓ DISPLAYED RECORDS CLOSED</span>",
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    ...
                    "font-size:.8rem;'>DISPLAYED RECORDS — OPEN AND CLOSED</span>",
                    unsafe_allow_html=True,
                )
            elif states == {"OPEN"}:
                st.markdown(
                    ...
                    "font-size:.8rem;'>OPEN RECORDS SHOWN</span>",
                    unsafe_allow_html=True,
                )

            if not message["rows"] and message["answer"].startswith("No open "):
                st.markdown("<span style='color:#22c55e;font-weight:700;'>✓ NO OPEN WORK FOUND — ACTIVE FILTERS</span>", unsafe_allow_html=True)
    ...
    records, coverage = _ta_retrieve(
        ...
        rfm_history=rfm_history,
    )
    coverage["mode"] = "matching history with current status" if history else "matched WOs/RFMs with current status"
    if not records:
        answer = _ta_no_results(coverage)
    elif not api_key:
