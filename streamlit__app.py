...
def render_turnover_assistant(wo_filtered, rfm_filtered, scope):
    ...
    if not records:
        ...
        try:
            ...
            # Do not expose credentials, request bodies, or remote error payloads.
            answer = "AI request failed. Check the API key, model access, quota and connection. Source rows are available below."
    messages = st.session_state.get("ta_messages", [])
    messages.append({"question": question, "answer": answer, "rows": records, "coverage": coverage})
